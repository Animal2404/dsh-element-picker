...
const execProbe = await page.evaluate(() => {
  ...
  editor.focus()
  // Built without escape sequences on purpose: this file is generated by
  // tooling often enough that a stray backslash must not be able to break it.
  const probeText = ['PROBE-A', 'PROBE-B', 'PROBE-C'].join(String.fromCharCode(10))
  const applied = document.execCommand('insertText', false, probeText)
